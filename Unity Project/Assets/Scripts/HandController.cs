...
public class HandController : MonoBehaviour {
  ...
  private void UpdateToolModels(Dictionary<int, ToolModel> all_tools,
                                ToolList leap_tools, ToolModel light, ToolModel spatula) {
    ...
    for (int h = 0; h < num_tools; ++h) {
      ...
      if (light && spatula) {
        ...
        if (!all_tools.ContainsKey(leap_tool.Id)) {
			if (leap_tools[0].Id > leap_tools[1].Id)
			{
				  ToolModel new_tool = CreateTool(light);
			...
			{
				ToolModel new_tool = CreateTool(spatula);
				new_tool.SetController(this);
				new_tool.SetLeapTool(leap_tool);
				new_tool.InitTool();
				all_tools[leap_tool.Id] = new_tool;
  ...
  void FixedUpdate() {
    ...
    UpdateToolModels(tools_, frame.Tools, toolModel, toolModel2);
  }
}
